...
/** Strip attachment noise for titles / API-friendly previews */
  ...
  const handleSend = async (overrideText?: string) => {
    ...
    try {
      ...
      if (reply && reply.trim()) {
        ...
        if (history.length === 0) {
          await refineConversationTitle(convId, content, reply)
        }
      } else {
                ...
                {showModelMenu && (
                  <>
                    ...
                    <motion.div
                      ...
                      className={`absolute left-1/2 -translate-x-1/2 top-full mt-1 z-[70] min-w-[220px] rounded-2xl py-1 shadow-lg border ${dark ? 'bg-[#16161f] border-white/10' : 'bg-white border-black/[0.06]'}`}
                    >
