...
function buildUserContent(text: string, files: PendingFile[]): string {
  ...
  for (const f of files) {
    if (f.type.startsWith('image/') && f.dataUrl) parts.push('![' + f.name + '](' + f.dataUrl + ')')
      ...
      parts.push('ðŸ“Ž **' + f.name + '**\n```\n' + body + '\n```')
    } else parts.push('ðŸ“Ž **' + f.name + '** _(file attached)_')
  }
  return parts.join('\n\n')
}
        ...
        for (const line of parts) {
          ...
          try {
            ...
            if (evt.status === 'tool_use') {
              // Tool calls can occur after Claude has already streamed useful text.
              // Keep that text visible while the server completes the tool loop.
              continue
