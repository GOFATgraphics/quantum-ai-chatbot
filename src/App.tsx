...
export default function App() {
  ...
  // Mic stream obtained on Speak tap (user gesture) so iOS allows it
  ...
  /** Open Live Voice — request mic in this user gesture (required on iPhone). */
  const openLiveVoice = async () => {
    try {
      ...
      if (Ctx) {
        ...
        // keep briefly; LiveVoice creates its own
    ...
    try {
      ...
      setErrorHint(
        e?.name === 'NotAllowedError'
          ? 'Microphone permission denied. Enable mic for this site in iPhone Settings → Safari.'
        ...
        <header className={'glass-header shrink-0 pt-[env(safe-area-inset-top)] ' + (showModelMenu ? 'z-50' : 'z-30')}>
          <div className="h-14 grid grid-cols-[1fr_auto_1fr] items-center px-3 gap-2">
            ...
            <div className="flex justify-center">
              <button ref={modelBtnRef} type="button" onClick={() => setShowModelMenu((v) => !v)} className={'glass-btn flex items-center gap-1.5 px-3 h-10 rounded-full text-[14px] font-medium transition ' + (dark ? 'text-slate-100' : 'text-slate-800')}>
                <span className="truncate max-w-[140px] sm:max-w-none">{selectedModel.name}</span>
                {selectedModel.badge ? <span className={'text-[10px] font-semibold px-1.5 py-0.5 rounded-md shrink-0 ' + (dark ? 'bg-white/10 text-slate-300' : 'bg-slate-100 text-slate-500')}>{selectedModel.badge}</span> : null}
          ...
          {showModelMenu && (
            <>
              ...
              <div className="fixed left-1/2 top-16 z-[70] -translate-x-1/2">
                <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} className={'glass-menu w-[min(260px,calc(100vw-24px))] rounded-2xl py-1 ' + (dark ? 'text-slate-100' : 'text-slate-800')}>
                  {MODELS.map((m) => (
                    <button key={m.id} type="button" onClick={() => selectModel(m)} className={'w-full text-left px-4 py-2.5 ' + (selectedModel.id === m.id ? (dark ? 'bg-white/8' : 'bg-black/[0.04]') : '')}>
                      <div className="text-sm font-medium">{m.name}</div>
                      <div className={'text-[12px] ' + (dark ? 'text-slate-500' : 'text-slate-400')}>{m.blurb}</div>
                    </button>
                  ))}
                </motion.div>
              </div>
        ...
        <div className="relative z-10 flex-1 flex flex-col min-h-0 overflow-hidden">
          {isEmpty ? (
            ...
            <main className="flex-1 overflow-y-auto min-h-0" data-scrollable="true">
              <MessageList messages={messages} isLoading={isLoading} lastUserPrompt={lastUserPrompt} dark={dark} messagesEndRef={messagesEndRef} thoughtSeconds={thoughtSeconds} thinkActive={false} deepSearchActive={false} onRegenerate={() => { if (lastUserPrompt) handleSend(lastUserPrompt) }} />
            </main>
          )}
          <div className="shrink-0">
            <ChatInput value={input} onChange={setInput} onSend={() => handleSend()} onStop={handleStop} onSpeak={() => void openLiveVoice()} language={voiceLanguage} isLoading={isLoading} dark={dark} errorHint={errorHint} pendingFiles={pendingFiles} onFilesChange={setPendingFiles} onFocusChange={setComposerFocused} />
          </div>
        </div>
      ...
      <AnimatePresence>
        {showLiveVoice && (
          <LiveVoice
            ...
            onAsk={askForVoice}
            initialStream={voiceStreamRef.current}
          />
        )}
      </AnimatePresence>
    </div>
