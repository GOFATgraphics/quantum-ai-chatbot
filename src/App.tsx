...
export default function App() {
  ...
  if (!session) {
    return <Auth dark={dark} onToggleDark={() => setDark((d) => !d)} />
  }
        ...
        <header className="pt-[env(safe-area-inset-top)] shrink-0 z-10">
          <div className="h-14 lg:h-16 flex items-center justify-between px-3 lg:px-6">
            <div className="flex items-center gap-2 min-w-0">
              {!isEmpty ? (
                <button onClick={startNewChat} className={`p-2 rounded-full transition lg:hidden shrink-0 glass-card ${dark ? 'glass-card-dark' : 'glass-card-light'}`}>
                  <ArrowLeft className={`w-5 h-5 ${textMuted}`} />
                </button>
              ) : (
                <button onClick={() => setMobileSidebar(true)} className={`p-2 rounded-full transition lg:hidden shrink-0 glass-card ${dark ? 'glass-card-dark' : 'glass-card-light'}`}>
              ...
              <div className="relative min-w-0">
                ...
                >
                  ...
                  {selectedModel.badge && <span className={`text-[10px] shrink-0 ${textMuted}`}>{selectedModel.badge}</span>}
            ...
            {isEmpty ? (
              ...
              >
                ...
                <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-2 w-full max-w-lg">
                  {['Summarize my unread emails', 'Find recent Drive files', 'Draft a short email', 'What should I focus on today?'].map((s) => (
                    <button
                      key={s}
          ...
          <div className="max-w-3xl mx-auto">
            <div className={`flex items-end gap-2 rounded-3xl px-3 py-2 glass-card ${dark ? 'glass-card-dark' : 'glass-card-light'}`}>
              <textarea
                ...
                className={`flex-1 resize-none bg-transparent border-0 outline-none text-[15px] leading-6 py-2 max-h-40 ${textMain} placeholder:${textMuted}`}
              />
              <button
                type="button"
                className={`p-2 rounded-full shrink-0 ${textMuted}`}
                aria-label="Voice"
              >
...
function Bubble({ message, dark }: { message: Message; dark: boolean }) {
  ...
  if (isUser) {
    return (
      <div className={`max-w-[85%] sm:max-w-[75%] rounded-3xl rounded-br-md px-4 py-2.5 text-[15px] leading-relaxed ${dark ? 'bg-indigo-600 text-white' : 'bg-indigo-600 text-white'}`}>
        {message.content}
      </div>
    )
  }
