...
export default function App() {
  ...
  const createProject = async (name: string, description?: string, color?: string) => {
    if (!user) return
    const { data, error } = await supabase.from('projects').insert({ user_id: user.id, name: name.trim(), description: description?.trim() || null, color: color || '#6366f1' }).select().single()
    ...
    const response = await fetch('/api/chat', {
      ...
      body: JSON.stringify({
        messages: [
          ...history.map((m) => ({
            role: m.role,
            content: m.role === 'user' ? String(m.content || '').replace(/!\[[^\]]*\]\(data:image\/[^)]+\)/g, '[Image attached]') : m.content,
          })),
                ...
                <div className="flex gap-2 w-max max-w-none mx-auto sm:flex-wrap sm:w-full sm:justify-center">
                  {EMPTY_STARTERS.map((s) => (
                    <button
                      ...
                      className={`text-[13px] font-medium px-3.5 py-2 rounded-full transition border whitespace-nowrap ${
                        dark
                          ? 'bg-white/5 border-white/10 text-slate-200 hover:bg-white/10 active:scale-[0.98]'
                          : 'bg-white border-slate-200 text-slate-700 hover:bg-slate-50 shadow-sm active:scale-[0.98]'
                      }`}
                    >
                      {s}
                    </button>
        ...
        {openProjectId && projects.find((p) => p.id === openProjectId) && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 bg-black/50" onClick={() => setOpenProjectId(null)}>
            <motion.div initial={{ y: 40 }} animate={{ y: 0 }} exit={{ y: 24 }} className="glass-sheet w-full sm:max-w-[880px] h-[min(94dvh,860px)] rounded-t-[28px] sm:rounded-[28px] overflow-hidden" onClick={(e) => e.stopPropagation()}>
              <ProjectDashboard dark={dark} user={user} project={projects.find((p) => p.id === openProjectId)!} conversations={conversations} onClose={() => setOpenProjectId(null)} onUpdateProject={updateProject} onDeleteProject={deleteProject} onSelectChat={selectChatFromDashboard} onNewChatInProject={() => newChatInProject(openProjectId)} />
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
