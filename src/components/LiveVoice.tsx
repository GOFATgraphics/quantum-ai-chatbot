...
/** Pull complete sentences from a growing stream buffer. */
function pullSentences(buffer: string): { sentences: string[]; rest: string } {
  ...
  while ((match = re.exec(rest)) !== null) {
    ...
    const piece = rest.slice(lastEnd, end).trim()
...
/** Queue sentences for TTS with prefetch of the next sentence while current plays. */
function createSentencePlayer(
  ...
  let rejectDone: ((e: any) => void) | null = null
  let settled = false
  ...
  const pump = async () => {
    ...
    try {
      while (queue.length > 0 || pendingBuf) {
        ...
        if (!buf) {
          ...
          const prefetch = nextText
            ? fetchTtsBuffer(nextText, language, signal).catch(() => null)
            : Promise.resolve(null)
          buf = await fetchTtsBuffer(sentence, language, signal)
          const prefetched = await prefetch
          if (prefetched && queue.length > 0) {
            queue.shift()
            pendingBuf = prefetched
          }
        }
        await playBuffer(buf, audioRef, signal)
      }
      if (finished) settleOk()
    } catch (e: any) {
      if (e?.name === 'AbortError') settleErr(e)
      else if (finished) settleErr(e)
    } finally {
      pumping = false
      if (finished && queue.length === 0 && !pendingBuf && !settled) settleOk()
    }
  }
  ...
  return {
    ...
    finish() {
      finished = true
      if (!pumping && queue.length === 0) settleOk()
      else void pump()
      return donePromise
    },
  }
...
export default function LiveVoice({
  ...
  const handleTurn = async (text: string) => {
    ...
    const player = createSentencePlayer(languageRef.current, audioRef, ac.signal)
    ...
    try {
      const answer = await onAsk(text, {
        ...
        onDelta: (delta) => {
          ...
          const cleaned = streamBuffer
            ...
            .replace(/\u2013/g, '-')
            .replace(/\u2015/g, ',')
          const { sentences, rest } = pullSentences(cleaned)
          if (sentences.length) {
            streamBuffer = rest
            if (!spokenStarted) {
              spokenStarted = true
            ...
            for (const s of sentences) {
              player.enqueue(s)
              setReply((prev) => (prev ? prev + ' ' : '') + s)
            }
          } else {
            setReply(cleaned.trim())
          }
        },
      })
      ...
      if (!spokenStarted && clean) {
        setPhase('speaking')
        setMicEnabled(false)
        player.enqueue(clean)
      }

      await player.finish().catch((e: any) => {
        if (e?.name !== 'AbortError') setError(e?.message || 'Could not play voice')
      })
    } catch (e: any) {
      ...
      setPhase('idle')
      setCaption('')
      scheduleListen(spokenStarted ? 280 : 700)
    }
  }
  ...
  const changeLanguage = (l: VoiceLanguage) => {
    setLanguage(l)
    onLanguageChange?.(l)
    try { localStorage.setItem('quantumy-language', l) } catch { /* ignore */ }
  }
  ...
  const statusLabel =
    phase === 'listening'
      ? language === 'ha' ? 'Ina saurare… yi magana yanzu' : 'Listening… speak now'
      : phase === 'thinking'
        ? language === 'ha' ? 'Ina tunani…' : 'Thinking…'
        : phase === 'speaking'
          ? language === 'ha' ? 'Ina magana…' : 'Speaking…'
          : language === 'ha'
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 pb-8">
        ...
        <p className={`text-center text-[15px] font-medium mb-3 ${dark ? 'text-white/80' : 'text-slate-700'}`}>{statusLabel}</p>
        ...
        <AnimatePresence mode="wait">
          {(caption || reply) && (
            <motion.p
              ...
              exit={{ opacity: 0 }}
              className={`text-center text-[17px] leading-relaxed max-w-md px-2 ${dark ? 'text-white' : 'text-slate-900'}`}
            >
        ...
        <div
          className={`mt-6 flex items-center gap-2 p-1 rounded-full ${dark ? 'bg-white/10' : 'bg-white/80 shadow-sm ring-1 ring-black/[0.04]'}`}
        ...
        >
          <button
            ...
            className={`h-9 px-3 rounded-full flex items-center gap-1.5 text-[13px] font-semibold transition ${
              language === 'en' ? (dark ? 'bg-white text-slate-900' : 'bg-slate-900 text-white') : dark ? 'text-white/70' : 'text-slate-600'
            }`}
          >
          ...
          <button
            ...
            className={`h-9 px-3 rounded-full flex items-center gap-1.5 text-[13px] font-semibold transition ${
              language === 'ha' ? (dark ? 'bg-white text-slate-900' : 'bg-slate-900 text-white') : dark ? 'text-white/70' : 'text-slate-600'
            }`}
          >
            Hausa
          </button>
