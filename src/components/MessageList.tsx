...
export default function MessageList({
  ...
  /** User is "stuck" to bottom unless they scroll up. */
  const stickToBottomRef = useRef(true)
  const lastConvRef = useRef<string | null>(null)
  const lastContentLenRef = useRef(0)

  // Track user scroll: leave stick mode when they scroll up; re-enter near bottom
  useEffect(() => {
  ...
  // Open conversation / first paint: always pin to bottom
  useEffect(() => {
  ...
  // Streaming / tool status / new tokens: stick only if user hasn't scrolled up
  useEffect(() => {
    if (messages.length === 0) return
    ...
    // Instant while streaming tokens (smooth fights rapid updates on mobile)
    const behavior: ScrollBehavior = isLoading && contentGrew ? 'auto' : isLoading ? 'auto' : 'smooth'
  ...
  // Keyboard / visualViewport resize: re-pin if stuck to bottom
  useEffect(() => {
    const reflow = () => {
  ...
  // Sending a new user message: re-enable stick + jump
  useEffect(() => {
    if (!isLoading) return
    // When loading starts, assume user wants to follow the reply
    stickToBottomRef.current = true
    scrollChatToBottomAfterLayout(messagesEndRef.current, { force: true, behavior: 'auto' })
