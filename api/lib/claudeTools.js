...
export async function runTool(block, user) {
  ...
  try {
    ...
    if (name === 'list_calendar_events' && user) {
      ...
      if (!token) return { type: 'tool_result', tool_use_id: id, content: JSON.stringify({ count: 0, error: 'Google Calendar is not connected.' }), is_error: true };
    ...
    // web_search / web_fetch are Anthropic server tools — never handled here
...
export async function loadConnectorsAndTools(user) {
  ...
  // Anthropic server tools always available; save_memory is our client tool
